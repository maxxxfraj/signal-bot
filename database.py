...
def save_active_signals(signals):
    ...
    try:
        ...
        for s in signals:
            tps = s.get('tps', [])
            tp_prices = [tp[0] for tp in tps] + [None] * 4
            tp_probs = [tp[1] for tp in tps] + [None] * 4
...
def close_signal_stat(signal_id, result, pct):
    ...
    try:
        ...
        cursor.execute('''
            ...
            WHERE id = %s
        ''', (result, pct, datetime.now().isoformat(), signal_id))
        conn.commit()
        cursor.close()
    except Exception as e:
